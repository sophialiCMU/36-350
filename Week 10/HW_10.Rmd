...
# FILL ME IN
df1 = apply(data.frame(sprint.lines), MARGIN=1 , FUN = function(x){x[x !=" "]}) %>%
  data.frame(.)
# df3 = lapply(df1, FUN = function(x){x[!x == ""]})
# print("hi")
gsub
# df2 = apply(data.frame(sprint.lines), MARGIN = 1, FUN= gsub, pattern = " ", replacement = "")
# df2
# lapply(df1[[1]], FUN=function(x){x[!x==""]})
  # lapply(., FUN = function(x){x[!x == ""]})
# t = strsplit(sprint.lines[1], " ")
# data.frame(lapply(t, function(x){x[!x == ""]}))
