...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/sophialiCMU/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/sophialiCMU/36-350/main/Week%208/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/sophialiCMU/36-350/main/Week%208/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/sophialiCMU/36-350/new_branch/Week%208/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/sophialiCMU/36-350-Fork/main/check_mark.R")
```
...
# test_that(desc = "Test for string of length greater than 1",expect_equal(f("aa"),0))
test_that(desc = "Test for vector of length > 1",           expect_error(f(c("a", "b"))))
test_that(desc = "Test for vector of length = 1",           expect_equal(f(c("ji")), log(10)))
test_that(desc = "Test for improper input (numeric)",       expect_error(f(1)))
test_that(desc = "Test for expected output length (1)",     expect_length(f("a"),1))
test_that(desc = "Test that output is integer",             expect_type(f("a"),"double"))
test_that(desc = "Test that upper-case letters work",            expect_equal(f("A"),0))
test_that(desc = "Test that the length of the input vector is 1",expect_error(f(letters)))
...
f = function(letter) {
  ...
  if ( nchar(letter) > 1 ) print("WARNING: The input character must be of length 1.")
  if ( nchar(letter) > 1) letter = substr(letter, 1, 1)
  return(log(which(letters==tolower(letter))))
}
