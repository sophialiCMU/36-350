...
\cd /Users/sop12/Downloads
\i player-stats.sql

postgres=# \cd /Users/sop12/Downloads
postgres=# \! cd
C:\Users\sop12\Downloads
...
```
select count (distinct college)as "count" from player_bios;

postgres=# select count (distinct college)as "count" from player_bios;
 count
-------
   125
(1 row)
...
postgres(#   name text,
postgres(#   division DivisionType,
postgres(#   coference ConferenceType
postgres(# );
CREATE TABLE
```
...
postgres=# create table new_table (
postgres(#   player integer references more_player_stats,
postgres(#   prl numeric,
postgres(#   position text
postgres(# );
CREATE TABLE
postgres=# insert into new_table (player, prl)
postgres-#   (select id, round(per - 67*va/(gp*minutes),1)
postgres(#   from more_player_stats);
INSERT 0 476
postgres=# update new_table set position = 'PF' where prl >= 11.3;
UPDATE 110
postgres=# update new_table set position = 'PG' where prl >= 10.8 and prl < 11.3;
UPDATE 92
postgres=# update new_table set position = 'C' where prl >= 10.6 and prl < 10.8;
UPDATE 80
postgres=# update new_table set position = 'SG/SF' where prl >= 0 and prl < 10.6;
UPDATE 193
