...
# Q: to calculate the returned time when the item was not returned, should I do the current date or the date it was loaned?
# returned[is.na(returned)] = loaned[is.na(returned)]

returned[is.na(returned)] = Sys.time()
days.out = as.numeric((returned - loaned)/(60*60*24))

hist = hist(days.out, xlim=c(0,1000), breaks = 10000/10, xlab = "Loan Duration (Days)", main=NULL, col="limegreen")
# !!!!!!!!!! CHECK FOR ITEM WAS RETURNED BEFORE IT WAS LOANED OUT!!!!!!!!!!!!
```
...
my.data = data.frame(x = hist$breaks[seq(1,100)], 
                     y = hist$counts[seq(1,100)])

##### TRY Something

my.data = hist$counts[seq(1,100)]
# load(url("http://www.stat.cmu.edu/~mfarag/350/Lab_07_Q10.Rdata"))
# my.data
my.fit.fun = function(my.par,my.data)
{
  -sum(log(dgamma(my.data,my.par[1],scale=my.par[2])))
}
out = optim(par=c(2,2),fn=my.fit.fun,my.data=my.data)

hist(my.data,breaks=seq(-0.5,max(my.data)+1,by=1),prob=TRUE,col="slateblue",main=NULL)
x = min(my.data):max(my.data)
lines(x,dexp(x,lambda.hat),lwd=3)
####
