...
#   }) %>% data.frame(Nums = .)
# 
# factored = data.frame(Num = as.factor(num$Nums)) %>%
#   filter(., . != 1)
# table(factored)[2:38]
# plot(table(factored)[2:38], xlab = "Number of Subjects", 
#      ylab="Occurances") # can use log="y" but it makes the graph bad

## use sapply
v2 = sapply(data.frame(sub = file.edit$Subjects), FUN = function(x){
  num = strsplit(x, split=";")
  return(length(num[[1]]))
  })
v2
